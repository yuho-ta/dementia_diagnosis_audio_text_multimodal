...
def set_splits():
    """
    K-Fold交差検証用のデータ分割を作成・保存（被験者IDベース）
    ...
    for index, row in labels_pd.iterrows():
        ...
        uids.append(uid)
        # 診断結果を数値に変換（cn=0, ad=1）
        labels.append(0 if diagno == "cn" else 1)
    ...
    for i, (train_index, test_index) in enumerate(sgkf.split(uids, labels, groups=subject_ids)):
        print(f"Fold {i}: TRAIN subjects: {len(set([subject_ids[j] for j in train_index]))}, TEST subjects: {len(set([subject_ids[j] for j in test_index]))}")
        print(f"Fold {i}: TRAIN samples: {len(train_index)}, TEST samples: {len(test_index)}")
        
        # 被験者IDの重複チェック
        train_subjects = set([subject_ids[j] for j in train_index])
        test_subjects = set([subject_ids[j] for j in test_index])
