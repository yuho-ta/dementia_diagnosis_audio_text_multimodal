...
# パス設定
root_path = os.path.join('dataset', 'diagnosis', 'test', 'audio')      # 音声ファイルのルートパス
root_text_path = os.path.join('dataset', 'diagnosis', 'test', 'text')  # テキスト埋め込みの出力パス

textual_data = os.path.join('dataset', 'diagnosis', 'test', 'text_transcriptions.csv')  # 書き起こしデータのCSV
...
def preprocess_text():
    ...
    for index, row in df.iterrows():
        ...
        logger.info(f"------------------------------------------")
        logger.info(f"------------------------------------------")
        logger.info(f"Processing {row['uid']}")

        # 出力ファイルのパス設定
        text_embedding_path = os.path.join(root_text_path, row['uid'] + textual_model_data + pauses_data + '.pt')
        audio_embedding_path = os.path.join(root_text_path, row['uid'] + textual_model_data + pauses_data + audio_model_data + '.pt')
        ...
        if audio_model != '':
            ...
            # eGeMAPS特徴量の場合
                ...
                if torch.isnan(features_audio).any():
                    logger.error(f"ERROR BEFORE in {row['uid']}: NaN values in features_audio")
            ...
            word_level_timestamp_path = os.path.join(root_text_path,row['uid'] + '.csv')
            ...
            if n_audio_segments + 2 != total_tokens:
                logger.debug(f"ERROR in {row['uid']}: Number of audio segments ({n_audio_segments}) does not match the number of tokens ({total_tokens})")
                logger.debug(f"Completed audios: {completed_audios}")
                return -1

            # NaN値のチェック
            if torch.isnan(processed_audio_tensor).any():
                logger.debug(f"ERROR in  {row['uid']}: NaN values in processed_audio_tensor")
                logger.debug(f"Completed audios: {completed_audios}")
                return -1
